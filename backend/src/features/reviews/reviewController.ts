...
export async function listReviewsForChurrasqueiro(
...
export async function createReview(req: AuthenticatedRequest, res: Response) {
  ...
  if (!bookingId || typeof rating !== "number") {
    return res
      .status(400)
      .json({ message: "bookingId e rating são obrigatórios" });
  }
  ...
  if (booking.userId !== req.user.sub && req.user.role !== "admin") {
    return res
      .status(403)
      .json({ message: "Você não pode avaliar este agendamento" });
  }
  ...
  if (existing) {
    return res
      .status(400)
      .json({ message: "Você já avaliou este agendamento" });
  }
