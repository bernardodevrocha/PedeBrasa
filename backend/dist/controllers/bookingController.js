...
async function createBooking(req, res) {
    ...
    const { churrasqueiroId, date, startTime, endTime, totalPrice } = req.body;
    if (!churrasqueiroId || !date || !startTime || !endTime || typeof totalPrice !== 'number') {
        return res.status(400).json({
            message: 'churrasqueiroId, date, startTime, endTime e totalPrice são obrigatórios',
        });
    }
